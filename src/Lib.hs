...
run (Opts workers verbose src dst) = do
 ...
 where
    ...
    doWithProgress findFunc actionFunc description = do
      ...
      displayConsoleRegions $ do
        pg <- newProgressBar def { pgWidth = 100
                                 ...
                                 , pgOnCompletion = Just $ "Done :percent after :elapsed seconds (" ++ T.unpack  description ++ ")"
                                 }
        mapPool_ workers (applyProgress pg actionFunc dst) (fmap SrcFilePath files)
        liftIO $ complete pg
    ...
    applyProgress pg f dst' src' = do
      createDirectoryIfMissing True $ takeDirectory (joinPath (fromDstFilePath dst') (fromSrcFilePath src'))
      f src' dst'
      barComplete <- isComplete pg
      unless barComplete $ tick pg

    copy :: SrcFilePath -> DstFilePath -> IO ()
    copy (SrcFilePath src') (DstFilePath dst') = copyFile src' $ joinPath dst' src'
